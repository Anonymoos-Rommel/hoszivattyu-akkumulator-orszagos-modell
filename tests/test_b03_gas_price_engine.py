...
class B03GasPriceEngineTests(unittest.TestCase):
    ...
    def test_regulated_schedule_keeps_threshold_and_prices_q(self) -> None:
        tariff = rows("residential_gas_tariff_schedule.csv")
        self.assertEqual({"DISCOUNTED_BAND", "ABOVE_THRESHOLD_BAND"}, {r["band"] for r in tariff})
        for row in tariff:
            self.assertEqual("1729", row["threshold_m3"])
            self.assertEqual("63645", row["threshold_mj"])
            self.assertEqual("Q", row["status"])
            self.assertEqual("", row["price_huf_per_m3"])
